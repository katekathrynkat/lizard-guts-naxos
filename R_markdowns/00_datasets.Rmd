...
diet_inverts <- diet %>% 
  ...
  filter(count != 0) %>% 
  mutate(sample = paste0(site, "-D", str_sub(lizard_id, 7)),
         sample_type = "diet") %>%
...
**Number of samples containing each order:**
...
**Number of samples containing each type of veg:**
...
## Combined data (trapping + diet)

### Abundance data

```{r}

inverts_all <- full_join(prey, diet_inverts) %>% 
